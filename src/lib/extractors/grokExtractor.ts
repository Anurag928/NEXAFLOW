...
export async function extractGrok(shareUrl: string, html: string): Promise<ExtractorResult> {
  const jsonObjects = extractJsonFromHtml(html);
  
  for (const data of jsonObjects) {
    const messages = findMessagesInAnyJSON(data);
    if (messages && messages.length > 0) {
      console.log(`[3] Extraction method used: Grok Script JSON`);
      return { success: true, messages };
    }
  }

  const browserMessages = await browserFallbackExtract(shareUrl, "grok");
  ...
  if (fallbackMessages.length > 0) {
    console.log(`[3] Extraction method used: Grok HTML Scrape Fallback`);
